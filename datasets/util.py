...
def change_coordinates(array, down_scale, offset, order = 0, preserve_range = True):#array(366, 500),down_scale=32
    ...
    pad = int(-offset + (down_scale - 1)/2)#15
    orig_h = array.shape[0]#366
    orig_w = array.shape[1]#500
    new_h = int(np.ceil(float(orig_h + 2 * pad) / down_scale))#13
    new_w = int(np.ceil(float(orig_w + 2 * pad) / down_scale))#17
    #floor or ceil?
    if pad > 0:
        pad_array = ((0,0),) * (len(array.shape) - 2) + ((pad, int(new_h * down_scale - orig_h - pad)), (pad, int(new_w * down_scale - orig_w - pad)))#((15,35),(15,29))
        new_array = np.pad(array, pad_array, 'constant')#(416, 544)
    elif pad == 0:
        new_array = array
    else:
        raise Exception
    ...
    if new_h != orig_h or new_w != orig_w:
        return resize(new_array, (new_h,new_w) + array.shape[2:], order = order, preserve_range = preserve_range)#相当于对mask进行下采样到32倍，下采样后的大小变为(13, 17)
    else:
...
class PASCAL:
    def __init__(self, db_path, dataType):
        ...
        self.db_path = db_path#'D:\\Project\\literature\\Oneshot\\codes\\data\\VOCdevkit2012\\VOC2012'
        ...
        self.name_id_map = dict(zip(classes, range(1, len(classes) + 1)))#每个类对应的标签号
        self.id_name_map = dict(zip(range(1, len(classes) + 1), classes))#每个标签号对应的类
        self.dataType = dataType
    ...
    def get_unique_ids(self, mask, return_counts=False, exclude_ids = [0, 255]):
        ids, sizes = np.unique(mask, return_counts=True)#对应mask所含的标签IDS[  0,   1, 255],每个标签对应的个数[143868,  30937,   8195]
        ids = list(ids)#[  0,   1, 255]
        sizes = list(sizes)#[143868,  30937,   8195]
        for ex_id in exclude_ids:#[0, 255]
          if ex_id in ids:#如果0和255在ids里面，则将0和255去掉,则剩下的ids=[1],sizes=[30937]
    ...
    def create_anns(self, read_mode):
        ...
        anns = []
        for item in names:
            mclass_path = osp.join(self.db_path, 'SegmentationClass', item + '.png')#编号对应图片的语义分割mask
            mobj_path = osp.join(self.db_path, 'SegmentationObject', item + '.png')#编号对应图片的实例分割mask
            mclass_uint = np.array(Image.open(mclass_path))#(366, 500)
            mobj_uint = np.array(Image.open(mobj_path))#(366, 500)
            class_ids = self.get_unique_ids(mclass_uint)#语义分割图片mask对应的类，将mask中0和255去掉，剩下的为图片所属的类,[1]
            obj_ids, obj_sizes = self.get_unique_ids(mobj_uint, return_counts = True)#实例分割图片mask对应的类，将mask中0和255去掉，剩下的为图片所属的类，[1,2,3],以及所属类在图片中的个数

            if read_mode == PASCAL_READ_MODES.INSTANCE:  #如果是实例分割
            ...
            elif read_mode == PASCAL_READ_MODES.SEMANTIC:#如果是语义分割
                for class_id in class_ids:#[1]
                    assert(class_id != 0 or class_id != 255)#
                    anns.append(dict(image_name=item, mask_name=item, class_ids=[class_id]))#将对应的图片编号和图片mask的编号，以及所属类别放入字典中
    ...
    def load_anns(self, read_mode):
        ...
        if not osp.exists(path):
            self.create_anns(read_mode)#将验证集对应的图片的编号以及mask编号和所属的类别放入一个.pkl文件中
        #path = path.encode()
        with open(path, 'rb') as f:
            anns = pickle.load(f)
        return anns#验证集对应的图片的编号以及mask编号和所属的类别
    
    def get_anns(self, catIds=[], areaRng=[], read_mode = PASCAL_READ_MODES.INSTANCE):
        if areaRng == []:
            areaRng = [0, np.inf]
        anns = self.load_anns(read_mode)#2148个验证集对应的图片的编号以及mask编号和所属的类别
        ...
        if read_mode == PASCAL_READ_MODES.INSTANCE:
            ...
            filtered_anns = []
            catIds_set = set(catIds)#[1,2,3,4,5]
            for ann in anns:#遍历每一个图片，ann={'image_name': '2007_000033', 'mask_name': '2007_000033', 'class_ids': [1]}
                class_inter = set(ann['class_ids']) & catIds_set#[1]&[1,2,3,4,5]=[1]取交集，判断当前图片是否属于catIds_set
                ...
                if len(class_inter) > 0:
                    ...
                    filtered_anns.append(ann)#将验证集中属于[1,2,3,4,5]的图片放入filtered_anns中
        return filtered_anns#440个
    
    def getItems(self, cats=[], areaRng=[], read_mode = PASCAL_READ_MODES.INSTANCE):
        if len(cats) == 0:
            catIds = self.id_name_map.keys()
        else:
            catIds = self.getCatIds(catNms=cats)#[1,2,3,4,5]
        catIds = np.sort(catIds)

        anns = self.get_anns(catIds=catIds, areaRng=areaRng, read_mode=read_mode)#440个，返回的是验证集中属于catIds=[1,2,3,4,5]的图片对应的编号以及mask编号，和对应的类别
        ...
        for i in range(len(anns)):#440
            ann = anns[i]#{'image_name': '2007_000033', 'mask_name': '2007_000033', 'class_ids': [1]}
            img_path = osp.join(self.db_path, 'JPEGImages', ann['image_name'] + '.jpg')#对应图片的位置
            if read_mode == PASCAL_READ_MODES.INSTANCE:#实例分割
                ...
                mask_path = osp.join(self.db_path, 'SegmentationClass', ann['mask_name'] + '.png')#对应图片的mask位置
    ...
    def cluster_items(items):
        clusters = {}
        for i, item in enumerate(items):#遍历图片，共365
            assert(isinstance(item, DBPascalItem))
            item_id = item.obj_ids#图片所属类别[1]，【4】
            assert(len(item_id) == 1), 'For proper clustering, items should only have one id'#判断图片是否只有一类
            item_id = item_id[0]
            if item_id in clusters:
                clusters[item_id].append(item)
            else:
              clusters[item_id] = DBImageSetItem('set class id = ' + str(item_id), [item])#将对应的类装入clusters中
        return clusters#共5类，将每一类图片放在一起
########################################################################### DB Items ###################################################################################
class DBVideoItem:
