...
class Gconv(nn.Module):
    ...
    def __init__(self, in_features, out_features):
        ...
        self.a_fc = nn.Linear(self.num_inputs, self.num_outputs)#[1, 2704, 256]
        self.u_fc = nn.Linear(self.num_inputs, self.num_outputs)

    def forward(self, A, x, norm=True):#[1, 2704, 2704][1, 2704, 512]
        if norm is True:
            A = F.normalize(A, p=1, dim=-2)

        ax = self.a_fc(x)#[1, 2704, 256]
        ux = self.u_fc(x)#[1, 2704, 256]
        x = torch.bmm(A, F.relu(ax)) + F.relu(ux) # has size (bs, N, num_outputs)[1, 2704, 256]
...
class Siamese_Gconv(nn.Module):
    ...
    def forward(self, g1, g2):
        emb1 = self.gconv(*g1)#[1, 2704, 256]
        emb2 = self.gconv(*g2)#[1, 2704, 256]
