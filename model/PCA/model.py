...
class Net(CNN):
    ...
    def forward(self, src, tgt, P_src, P_tgt, G_src, G_tgt, H_src, H_tgt, ns_src, ns_tgt, K_G, K_H, type='img'):
        ...
        # adjacency matrices
        A_src = torch.bmm(G_src, H_src.transpose(1, 2))#[8, 10, 10]
        #print('A_src', A_src.shape)
        A_tgt = torch.bmm(G_tgt, H_tgt.transpose(1, 2))#[8, 10, 10]
        #print('A_tgt', A_tgt.shape)
        emb1, emb2 = torch.cat((U_src, F_src), dim=1).transpose(1, 2), torch.cat((U_tgt, F_tgt), dim=1).transpose(1, 2)
        #print('emb1', emb1.shape)#[8, 10, 1024]
        #print('emb2', emb2.shape)#[8, 10, 1024]
        #print(self.gnn_layer)#2
        for i in range(self.gnn_layer):
            gnn_layer = getattr(self, 'gnn_layer_{}'.format(i))
            print(gnn_layer)
            emb1, emb2 = gnn_layer([A_src, emb1], [A_tgt, emb2])
            print('emb11', emb1.shape)#[8, 10, 2048]
            print('emb22', emb2.shape)#[8, 10, 2048]
            affinity = getattr(self, 'affinity_{}'.format(i))
            s = affinity(emb1, emb2)#[8, 10, 10]
            print('s', s.shape)
            s = self.voting_layer(s, ns_src, ns_tgt)#[8, 10, 10]
            print('s', s.shape)
            s = self.bi_stochastic(s, ns_src, ns_tgt)#[8, 10, 10]
            print('s', s.shape)
            if i == self.gnn_layer - 2:
                cross_graph = getattr(self, 'cross_graph_{}'.format(i))
                emb1_new = cross_graph(torch.cat((emb1, torch.bmm(s, emb2)), dim=-1))
                print('emb1_new', emb1_new.shape)#[8, 10, 2048]
                emb2_new = cross_graph(torch.cat((emb2, torch.bmm(s.transpose(1, 2), emb1)), dim=-1))
                emb1 = emb1_new
                print('emb2_new', emb2_new.shape)#[8, 10, 2048]
                emb2 = emb2_new
                print('emb2_new', emb2_new.shape)#[8, 10, 2048]
