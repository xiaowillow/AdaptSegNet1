...
class DBInterface():
    def __init__(self, params):
        self.lock = Lock()
        self.params = params
        self.load_items()#5类图片对应365张，对应的类别以及索引
    ...
    def _remove_small_objects(self, items):
        ...
        for item in items:
            ...
            if util.change_coordinates(mask, 32.0, 0.0).sum() > 2:##相当于对mask进行下采样到32倍，下采样后的大小变为(13, 17)，然后判断下采样的图片的像素值之和是否大于2
    ...
    def load_items(self):
        ...
        if ('image_sets') in self.params:
            for image_set in self.params['image_sets']:
                ...
                if image_set.startswith('pascal') or image_set.startswith('sbd'):
                    ...
                    #reads pair of images from one semantic class and and with binary labels
                        ...
                        items = pascal_db.getItems(self.params['pascal_cats'], self.params['areaRng'],
                                                   read_mode = util.PASCAL_READ_MODES.SEMANTIC)#将对应的5类验证集图片信息放在一起，共440张
                        items = self._remove_small_objects(items)#删掉小的目标后，剩下365张
                    else:
                        raise Exception('Only single_image and image_pair mode are supported')
            ...
            if self.params['output_type'] == 'image_pair':
                items = self.db_items#对应5类的验证集图片共365张
                
                #In image_pair mode pair of images are sampled from the same semantic class
                clusters = util.PASCAL.cluster_items(self.db_items)#将365张图片相同的类放在一起
                ...
                self.db_items = []
                for item in items:#365
                    set_id = item.obj_ids[0]#所属类别，[1]
                    imgset = clusters[set_id]#找到第一类所属的集合
                    assert(imgset.length > self.params['k_shot']), 'class ' + imgset.name + ' has only ' + imgset.length + ' examples.'#判断该类别总数是否大于所需的shot
                    in_set_index = imgset.image_items.index(item)#该张图片在对应类的集合中所处的位置，比如说，第1张，则对应的索引为0
                    self.db_items.append((imgset, in_set_index))#将对应图片和所属类别的索引放在一起,共365张
                cprint('Total of ' + str(len(clusters)) + ' classes!', bcolors.OKBLUE)#共5类
        ...
        self.seq_index = len(self.db_items)#365
...
class PairLoaderProcess(Process):
    def __init__(self, name, queue, db_interface, params):
        ...
        self.bgr = params['bgr']#False
        self.scale_256 = params['scale_256']#False
        self.first_label_mean = params['first_label_mean']#0
        self.first_label_scale = 1.0 if not ('first_label_scale') in params else params['first_label_scale']#1
        ...
        self.deploy_mode = params['deploy_mode'] if ('deploy_mode') in params else False#true
        self.has_cont = params['has_cont'] if ('has_cont') in params else False#False
        if self.bgr:
    ...
    def __prepross(self, frame_dict, shape = None):
        ...
        label = frame_dict['mask']#(375, 500)

        mean_vals = [0.485, 0.456, 0.406]
        std_vals = [0.229, 0.224, 0.225]
        mean_vals = np.array(mean_vals)
        std_vals = np.array(std_vals)
        image = (image-mean_vals)/std_vals#(375, 500, 3)
...
class SSDatalayer():
    def __init__(self, test_group, k_shot=1):
        ...
        self.param_str = {
            ...
            'batch_size': 1,
            'first_shape': [416,416],
            'second_shape': [416,416],
            # 'k_shot':1,
            'k_shot': k_shot,
            'has_cont': False,
            'deploy_mode': True,
            #'pascal_path':'/home/xiaolin/xlzhang/data/VOC2012'
            # 'pascal_path': '/home/ease/Documents/data/VOCdevkit2012/VOC2012'
            'pascal_path': 'D:\Project\literature\SemanticSegmentation\codes\data\VOCdevkit2012\VOC2012'
        }
    ...
    def setup(self):
        ...
        self.db_interface = DBInterface(params)#处理图片，根据对应类别将图片选取出来，以及相同类的图片在所占类中的位置索引等
        self.processes = []
        for i in range(params['worker_num']):
            process = PairLoaderProcess('SSDatalayer Process', self.queue, self.db_interface, params)#成对获取图片
            process.daemon = True
            process.start()
