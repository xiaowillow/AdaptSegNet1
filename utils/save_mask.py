...
def mask_to_img(mask, img):#mask(333, 500)img(333, 500, 3)
    mask_decoded = decode_labels(mask)#将mask转化成彩色图像

    # heat_map = cv2.applyColorMap(atten_norm.astype(np.uint8), cv2.COLORMAP_JET)
    img = cv2.addWeighted(img.astype(np.uint8), 0.5, mask_decoded.astype(np.uint8), 0.9, 0)#将原图和mask加权融合
    return img
